...
def video_cutting(mode, progress=gr.Progress()):
    if mode == "Stretch/narrow video to fit phrases (takes a lot of time)":
        # создание директорий
        if os.path.exists(dfunc.path_to_fragments):
            shutil.rmtree(dfunc.path_to_fragments)
            os.makedirs(dfunc.path_to_fragments)
        else:
            os.makedirs(dfunc.path_to_fragments, exist_ok=True)
        if os.path.exists(dfunc.path_to_intermediate):
            shutil.rmtree(dfunc.path_to_intermediate)
        ...
        if os.path.isfile(tf.path_to_video) and os.path.isfile(tf.path_to_text):
            ...
            command = ["ffmpeg", "-y", "-i", tf.path_to_video, "-to", af.str_to_time(lines[2][:lines[2].find(' ')]),
...
def make_dubbing(mode, slow_aud, limit, progress=gr.Progress()):
    times = []
    ...
    if os.path.exists(tf.path_to_init) and os.path.isfile(tf.path_to_text):
        ...
        if mode == "Simple audio overlay by timings":
            if os.path.isfile(tf.path_to_video):
                path_to_screensaver = 's.mp4'
                # вырезание заставки из видео
                progress(0.1, desc="Copying the screensaver...")
                command = ["ffmpeg", "-y", "-i", tf.path_to_video, "-to",
                           af.str_to_time(lines[2][:lines[2].find(' ')]), "-async", str(1),
                           path_to_screensaver]
                subprocess.run(command)
                # основная логика
        ...
        command = ["ffmpeg", "-i", tf.path_to_video, "-i", "synthesized/synthesized_speech1.wav", "-c:v", "copy", "-c:a",
